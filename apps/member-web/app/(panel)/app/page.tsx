...
export default async function AppHome() {
  ...
  const [ov, gm] = await Promise.all([authed<any>('/me/overview'), authed<any>('/me/gamification')]);
  return (
    <div className="stack" style={{ ['--stack' as string]: '28px' }}>
      ...
      <div className="stat-grid">
        <div className="stat-tile"><span className="n gradient-text">{gm.xp}</span><span className="l">Toplam XP · Seviye {gm.level}</span></div>
        <div className="stat-tile"><span className="n" style={{ color: 'var(--color-primary)' }}>🔥 {gm.streak.current}</span><span className="l">Günlük seri (en uzun {gm.streak.longest})</span></div>
        <Link href="/app/programs" className="stat-tile"><span className="n">{ov.subscriptions.length}</span><span className="l">Aktif abonelik · Programlarım</span></Link>
        <Link href="/app/health" className="stat-tile"><span className="n">❤</span><span className="l">Sağlık &amp; ilerleme</span></Link>
      </div>
      <div className="grid grid-3">
        <Link href="/app/messages" className="card card-hover stat-card"><MessageSquare className="text-primary-c" aria-hidden /><span className="n">Mesajlar</span><span className="caption text-tertiary">Abone olduğun koçlarla yaz</span></Link>
        <Link href="/app/notifications" className="card card-hover stat-card"><Bell className="text-primary-c" aria-hidden /><span className="n gradient-text">{ov.unreadNotifications}</span><span className="caption text-tertiary">Okunmamış bildirim</span></Link>
        <Link href="/app/support" className="card card-hover stat-card"><LifeBuoy className="text-primary-c" aria-hidden /><span className="n gradient-text">{ov.openTickets}</span><span className="caption text-tertiary">Açık destek talebi</span></Link>
      </div>
      <section>
        <h2 className="h4" style={{ marginBottom: 14 }}>Aboneliklerim</h2>
        {ov.subscriptions.length ? (
          <div className="grid grid-3">
            {ov.subscriptions.map((sub: any, i: number) => sub.coach && (
