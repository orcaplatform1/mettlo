...
export default async function CoachYoga({ searchParams }: { searchParams: Promise<{ member?: string }> }) {
  ...
  return (
    <div className="stack" style={{ ['--stack' as string]: '22px', maxWidth: 1000 }}>
      <div>
        <h1 className="h2 row" style={{ gap: 10 }}><Leaf className="text-primary-c" aria-hidden /> Yoga & Esneklik Koçluğu</h1>
        <p className="text-secondary body-sm">Üyelerinin yoga pratiğini ve esneklik gelişimini buradan takip edebilirsin. Program kaydı, antrenman geçmişi ve koçluk notlarına erişebilirsin.</p>
      </div>
      ...
      {!d && (clients.length === 0
        ...
        : (
          <div className="grid grid-3">
            {clients.map((c) => (
              <Link key={c.member.id} href={`/creator/yoga?member=${c.member.id}`} className="card card-hover row" style={{ gap: 14 }}>
      ...
      {d && (
        <>
          <div className="row between row-wrap">
            ...
            <Link href="/creator/yoga" className="btn btn-secondary btn-sm">← Üyeler</Link>
          </div>
            ...
            <section className="card stack" style={{ ['--stack' as string]: '10px' }}>
              ...
              {d.checkins.slice(0, 10).map((c: any) => (
                <div key={c.id} style={{ borderTop: '1px solid var(--border-soft)', paddingTop: 8 }}>
                  <div className="row between"><span className="body-sm"><b>Enerji:</b> {c.energyLevel}/5 · <b>Uyku:</b> {c.sleepHours ?? '—'} saat · <b>Stres:</b> {c.stressLevel ?? '—'}/5</span><span className="caption text-tertiary">{new Date(c.createdAt).toLocaleDateString('tr-TR')}</span></div>
          ...
          {d.practiceLogs && d.practiceLogs.filter((l: any) => l.branch === 'yoga-mobility').length > 0 && (
            <section className="card stack" style={{ ['--stack' as string]: '10px' }}>
              <h3 className="h5">Yoga Seans Kayıtları</h3>
              <div className="table-wrap">
                <table className="table">
                  <thead><tr><th>Tarih</th><th>Tip</th><th>Süre</th><th>Ruh Hali</th><th>Notlar</th></tr></thead>
                  <tbody>
                    {d.practiceLogs.filter((l: any) => l.branch === 'yoga-mobility').slice(0, 20).map((l: any) => (
                      <tr key={l.id}>
